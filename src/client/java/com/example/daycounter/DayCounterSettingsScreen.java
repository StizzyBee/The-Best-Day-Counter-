...
public class DayCounterSettingsScreen extends Screen {
   ...
   protected void init() {
      ...
      this.langDisplayBtn = (Button)this.addRenderableWidget(
         Button.builder(Component.literal(this.language.displayName), b -> {}).bounds(startX + smallW + gap, langRowY, midW, 20).build()
      );
      ...
      this.fontDisplayBtn = (Button)this.addRenderableWidget(
         Button.builder(Component.literal(this.currentFont.displayName), b -> {}).bounds(startX + smallW + gap, fontRowY, midW, 20).build()
      );
      ...
      this.caseDisplayBtn = (Button)this.addRenderableWidget(
         Button.builder(Component.literal(this.currentCase.displayName), b -> {}).bounds(startX + smallW + gap, caseRowY, midW, 20).build()
      );
      ...
      this.scaleDisplayBtn = (Button)this.addRenderableWidget(
      ...
      DayCounterSettingsScreen.Preset[] presets = DayCounterSettingsScreen.Preset.values();
      int presetTotalW = presets.length * btnW + (presets.length - 1) * 8;
      int presetStartX = (this.width - presetTotalW) / 2;

      for (int i = 0; i < presets.length; i++) {
         DayCounterSettingsScreen.Preset p = presets[i];
         this.addRenderableWidget(
   ...
   private void applyPreset(DayCounterSettingsScreen.Preset p) {
      int x = p.rawX;
      int y = p.rawY;
      switch (p) {
         case TOP_CENTER:
